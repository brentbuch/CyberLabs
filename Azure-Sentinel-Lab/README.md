...
This lab is meant to familiarize with Azure and Azure Sentinel. We will be setting up an instance of Azure Sentinel as well as a honeypot VM exposed to the internet. Logs will be ingested to Azure Log Analytics Workspace. The goal is to monitor incoming activity to the honeypot and plot the threats on a world map based on originating location. For the purposes of shortening this lab, the Azure account has already been created.
